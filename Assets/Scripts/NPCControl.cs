...
public class NPCControl : MonoBehaviour
{
    ...
    void Update()
    {
		...
		float range = abilities.skills[0].useDist;
		float angle = abilities.skills[0].useAngle;
		Spot();
		if(targets.Count > 0)
		{
			Abilities t = targets[0];
			...
			foreach (Collider col in Physics.OverlapSphere(checkAttack.position, checkAttackRadius, targetMask))
			{
